...
Subcommands: 
 ...
 Digital Signature:
  ...
  edgetk -tcp client -cert cert.pem -key priv.pem [-ipport "[2001::1]:8081"]`)
	...
	if (*pkey == "keygen") && (*alg != "sm9encrypt" && *alg != "sm9sign" && *alg != "bn256" && *alg != "bn256sign" && *alg != "bn256encrypt" && *alg != "bls12381" && *alg != "bls12381v" && *alg != "bls12381sign" && *alg != "bls12381encrypt" && *alg != "egphp" && *alg != "elgamalphp") && *pwd == "" {
		print("Passphrase: ")
	...
	if (*pkey == "keygen" || *pkey == "keygen-blinded") && (*alg == "bn256" || *alg == "bn256encrypt" || *alg == "bn256sign" || *alg == "bls12381" || *alg == "bls12381v" || *alg == "bls12381sign" || *alg == "bls12381encrypt") && *pwd == "" {
		file, err := os.Open(*master)
		if err != nil {
			log.Fatal(err)
		}
	...
	if (*pkey == "keygen" || *pkey == "keygen-blinded") && (*alg == "bn256" || *alg == "bn256encrypt" || *alg == "bn256sign" || *alg == "bls12381" || *alg == "bls12381v" || *alg == "bls12381sign" || *alg == "bls12381encrypt") && *pwd2 == "" {
		print("UserKey Passphrase: ")
		pass, _ := gopass.GetPasswdMasked()
		*pwd2 = string(pass)
		...
		if data, err := ioutil.ReadFile(*key); err == nil {
			if block, _ := pem.Decode(data); block != nil {
				if strings.Contains(block.Type, "SLH-DSA") {
					...
					*alg = "BLS12381I"
				} else if strings.Contains(block.Type, "BLS12381V") {
					*alg = "BLS12381V"
	...
	if *pkey == "derive" && strings.ToUpper(*alg) != "GOST2012" && strings.ToUpper(*alg) != "BN256" && strings.ToUpper(*alg) != "BN256I" && strings.ToUpper(*alg) != "BLS12381" && strings.ToUpper(*alg) != "BLS12381I" && strings.ToUpper(*alg) != "BLS12381V" {
	...
	if (*pkey == "text" || *pkey == "modulus" || *pkey == "check" || *pkey == "randomart" || *pkey == "fingerprint" || *pkey == "info") && *crl == "" && *params == "" {
		...
		if strings.Contains(s, "RSA PRIVATE") {
			...
			*alg = "BLS12381I"
		} else if strings.Contains(s, "BLS12381V") {
			*alg = "BLS12381V"
	...
	if (strings.ToUpper(*alg) == "BLS12381" || strings.ToUpper(*alg) == "BLS12381SIGN" || strings.ToUpper(*alg) == "BLS12381ENCRYPT") && (*pkey == "setup" || *pkey == "keygen" || *pkey == "sign" || *pkey == "verify" || *pkey == "proof" || *pkey == "verify-proof" || *pkey == "derive" || *pkey == "encrypt" || *pkey == "decrypt" || *pkey == "text" || *pkey == "fingerprint" || *pkey == "randomart") {
		var blockType string
		if *key != "" {
			pemData, err := ioutil.ReadFile(*key)
			if err != nil {
		...
		if *pkey == "keygen" && strings.ToUpper(*alg) == "BLS12381" {
			...
			if verifySM(&pubKey, []byte(*id), msg, decodedH, decodedSig) {
				fmt.Println("Verified: true")
			} else {
				fmt.Println("Verified: false")
				os.Exit(1)
			...
			ciphertext, err := EncryptBB(*id, msg, Ppub, Q, v)
			if err != nil {
			...
			U, V, W := encryptFO(&pubKey, []byte(*id), []byte(msg), myHash)
			...
			U, V, T := encryptBF(&pubKey, []byte(*id), []byte(msg), myHash)
			...
			if verifyProofShangMi(mpk, []byte(*id), msg, chi, S) {
				fmt.Println("Verified: true")
			} else {
				fmt.Println("Verified: false")
				os.Exit(1)
	...
	if (strings.ToUpper(*alg) == "BN256" || strings.ToUpper(*alg) == "BN256SIGN" || strings.ToUpper(*alg) == "BN256ENCRYPT") && (*pkey == "keygen" || *pkey == "setup" || *pkey == "sign" || *pkey == "verify" || *pkey == "encrypt" || *pkey == "decrypt" || *pkey == "text" || *pkey == "fingerprint" || *pkey == "randomart") {
		...
		if *pkey == "setup" {
			...
			if verifySM_BN(&pubKey, []byte(*id), msg, h, signature) {
				fmt.Println("Verified: true")
			} else {
				fmt.Println("Verified: false")
				os.Exit(1)
			...
			h, signature := signCC_BN(skG1, []byte(*id), msg)
			...
			U, V, W := encryptFO_BN(&pubKey, []byte(*id), msg, myHash)
			...
			U, V, T := encryptBF_BN(&pubKey, []byte(*id), msg, myHash)
	...
	if (strings.ToUpper(*alg) == "BLS12381V") && (*pkey == "keygen" || *pkey == "keygen-blinded" || *pkey == "setup" || *pkey == "sign" || *pkey == "sign-blinded" || *pkey == "aggregate" || *pkey == "aggregate-proof" || *pkey == "aggregate-signatures" || *pkey == "verify-aggregate" || *pkey == "aggregate-vote" || *pkey == "aggregate-vote-encrypted" || *pkey == "aggregate-vote-audit" || *pkey == "aggregate-vote-proof" || *pkey == "verify-aggregate-vote" || *pkey == "proof" || *pkey == "verify-proof" || *pkey == "verify-proof-message" || *pkey == "blind" || *pkey == "unblind" || *pkey == "verify-blinded" || *pkey == "unblind-signature" || *pkey == "count" || *pkey == "input" || *pkey == "count-total" || *pkey == "add" || *pkey == "sum" || *pkey == "hash" || *pkey == "verify" || *pkey == "derive" || *pkey == "encrypt" || *pkey == "decrypt" || *pkey == "text" || *pkey == "fingerprint" || *pkey == "randomart") {
		...
		if *pkey == "text" && *key != "" && (blockType == "BLS12381V SECRET KEY" || blockType == "BLS12381V BLINDED SECRET KEY" || blockType == "BLS12381V MASTER KEY") {
			keyBytes, err := readKeyFromPEM(*key, true)
			if err != nil {
				fmt.Println("Error loading key:", err)
				os.Exit(1)
			...
			if blockType == "BLS12381V MASTER KEY" {
				keyPEM := pem.Block{Type: "BLS12381V MASTER KEY", Bytes: keyBytes}
				keyPEMText := string(pem.EncodeToMemory(&keyPEM))
				fmt.Print(keyPEMText)
				fmt.Println("MasterKey:")
				p := fmt.Sprintf("%x", keyBytes)
			...
			} else if blockType == "BLS12381V SECRET KEY" {
				keyPEM := pem.Block{Type: "BLS12381 SECRET KEY", Bytes: keyBytes}
				keyPEMText := string(pem.EncodeToMemory(&keyPEM))
				fmt.Print(keyPEMText)
				fmt.Println("SecretKey:")
				...
				keyPEM := pem.Block{Type: "BLS12381V BLINDED SECRET KEY", Bytes: keyBytes}
				keyPEMText := string(pem.EncodeToMemory(&keyPEM))
				fmt.Print(keyPEMText)
				fmt.Println("SecretKey:")
				p := fmt.Sprintf("%x", keyBytes)
			...
			os.Exit(0)
		} else if *pkey == "text" && *key != "" && (blockType == "BLS12381V PUBLIC KEY") {
			keyBytes, err := readKeyFromPEM(*key, false)
			if err != nil {
				fmt.Println("Error reading key from PEM:", err)
				os.Exit(1)
			}
			pubKeyPEM := pem.Block{Type: "BLS12381V PUBLIC KEY", Bytes: keyBytes}
			keyPEMText := string(pem.EncodeToMemory(&pubKeyPEM))
			fmt.Print(keyPEMText)
			fmt.Println("PublicKey:")
			p := fmt.Sprintf("%x", keyBytes)
		...
		if *pkey == "setup" {
			...
			block := &pem.Block{
				Type:  "BLS12381V BLINDED SECRET KEY",
			...
			fmt.Println("BLS12381V("+inputdesc+")=", hex.EncodeToString(signature.BytesCompressed()))
		} else if *pkey == "verify" {
			pk, err := readKeyFromPEM(*key, false)
			if err != nil {
				fmt.Println("Error loading key:", err)
			...
			if *sig == "" {
				fmt.Println("Missing signature (use -signature)")
				os.Exit(1)
			}
			...
			err = blindedSignature.SetBytes(sigBytes)
			if err != nil {
				fmt.Println("Error deserializing signature:", err)
				os.Exit(1)
			}
			...
			signature := signMessageBLS(originalG1, skScalar)
			fmt.Println("Individual_BLS12381V("+inputdesc+")=", hex.EncodeToString(signature.BytesCompressed()))
			...
			fmt.Println("Aggregated_BLS12381V=", hex.EncodeToString(aggregatedSignature.BytesCompressed()))
			...
			fmt.Printf("HashG1("+inputdesc+")= %x\n", originalG1.BytesCompressed())
			fmt.Println("Blind_Factor=", hex.EncodeToString(blindFactorBytes))
			fmt.Println("Blind_Message=", hex.EncodeToString(blindedMessage.BytesCompressed()))
			...
			if *candidates != "" {
				...
				for _, candidate := range candidatesList {
					...
					if bytes.Equal(originalG1.BytesCompressed(), candidateHash.BytesCompressed()) {
		...
		} else if *pkey == "verify-proof-message" {
			pk, err := readKeyFromPEM(*key, false)
			if err != nil {
				fmt.Println("Error loading key:", err)
				os.Exit(1)
			...
			originalG1 := hashToG1(newVoteData)
			...
			fmt.Printf("Blind= %x\n", blindedMessage.BytesCompressed())
			...
			fmt.Printf("HashG1= %x\n", originalMessage.BytesCompressed())
		...
		} else if *pkey == "unblind-signature" {
			if *factorb == "" {
				fmt.Println("You must provide the blinding factor.")
				os.Exit(1)
			}
			...
			blindedSigBytes, err := hex.DecodeString(*sig)
			if err != nil {
				fmt.Println("Error decoding hexadecimal signature:", err)
				os.Exit(1)
			}
			...
			blindedSignature := new(bls12381.G1)
			err = blindedSignature.SetBytes(blindedSigBytes)
			if err != nil {
				fmt.Println("Error deserializing blinded signature:", err)
				os.Exit(1)
			}
			...
			unblindedSignature := new(bls12381.G1)
			unblindedSignature.ScalarMult(inverseBlindFactor, blindedSignature)

			fmt.Printf("Unblinded_Signature= %x\n", unblindedSignature.BytesCompressed())
		} else if *pkey == "derive" && *pub2 == "" {
			sk, err := readKeyFromPEM(*key, true)
			if err != nil {
				fmt.Println("Error loading key:", err)
...
func encryptBN(message string, publicKey *bn256i.G2, hashFunc func() hash.Hash) (*bn256i.G1, []byte, []byte) {
	...
	for i := range messageBytes {
		encrypted[i] = messageBytes[i] ^ S1[i%len(S1)]
	}
...
func decryptBN(C1 *bn256i.G1, authTag, encrypted []byte, privateKey *big.Int, hashFunc func() hash.Hash) string {
	...
	for i := range encrypted {
		message[i] = encrypted[i] ^ S1[i%len(S1)]
	}
...
func generatePrivateKeyForUserBLS(masterKey *ff.Scalar, userID string) *ff.Scalar {
	userScalar := hashToScalar([]byte(userID))
...
func verifyBlindSignature(blindedPointBytes []byte, signature *bls12381.G1, pubKey *bls12381.G2) bool {
	point := new(bls12381.G1)
	if err := point.SetBytes(blindedPointBytes); err != nil {
		return false
	}
	e1 := bls12381.Pair(signature, bls12381.G2Generator())
	e2 := bls12381.Pair(point, pubKey)
...
func verifyAggregateSignatureVote(blindedMessages []*bls12381.G1, aggSignature *bls12381.G1, pubKeys []*bls12381.G2) bool {
	...
	e2 := bls12381.Pair(blindedMessages[0], pubKeys[0])
	for i := 1; i < len(blindedMessages); i++ {
		e2.Mul(e2, bls12381.Pair(blindedMessages[i], pubKeys[i]))
	}
...
func H4(sigma []byte, hashFunc func() hash.Hash) []byte {
	h := hashFunc()
	h.Write([]byte("H4|"))
	h.Write(sigma)
	return h.Sum(nil)
...
func encryptFO(Ppub *bls12381.G2, id, M []byte, hashFunc func() hash.Hash) (*bls12381.G2, []byte, []byte) {
	...
	h4sigma := H4(sigma, hashFunc)
	W := make([]byte, len(M))
	for i := 0; i < len(M); i++ {
		W[i] = M[i] ^ h4sigma[i%len(h4sigma)]
	}
...
func H4_BN(sigma []byte, hashFunc func() hash.Hash) []byte {
	h := hashFunc()
	h.Write([]byte("H4|"))
	h.Write(sigma)
	return h.Sum(nil)
...
func EncryptBB(id string, msg []byte, Ppub, Q *bls12381.G1, v *bls12381.Gt) ([]byte, error) {
	...
	if _, err := rand.Read(sigma); err != nil {
		return nil, fmt.Errorf("falha ao gerar sigma: %v", err)
	}
	...
	symKey := sha256.Sum256(sigma)

	vs := new(bls12381.Gt)
	vs.Exp(v, s)

	vsBytes, _ := vs.MarshalBinary()
	pad := sha256.Sum256(vsBytes)
	...
	if err != nil {
		return nil, fmt.Errorf("falha ao codificar ASN.1: %v", err)
	}
...
func DecryptBB(ct []byte, r *ff.Scalar, Did *bls12381.G2, v *bls12381.Gt) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("falha ao decodificar ASN.1 do ciphertext: %v", err)
	}
	...
	if len(A) != 32 || len(Bbytes) != 96 || len(C1bytes) != 96 {
		return nil, fmt.Errorf("componentes do ciphertext inválidos")
	}
	...
	vs.Exp(v, s)
	vsBytes, _ := vs.MarshalBinary()
	checkPad := sha256.Sum256(vsBytes)
...
func encodeVotesToASN1(votes []*bls12381.G1) (string, error) {
	...
	for i, vote := range votes {
		encoded.Votes[i] = vote.BytesCompressed()
	}
