...
2. The public key is generated as: 
   $P_{\text{pub}} = x \cdot P$ 
   where $P$ is a generator point of the elliptic curve in group $G_1$.
...
4. **Signature Generation:** The signature $(R, s)$ is generated with the formula 
   $s = k - x \cdot h \pmod{r}$
...
2. Verify the following pairing equation:
   $e(R, P_{\text{pub}}) = e(P, Q) \cdot e(P, R)^{-h}$
   where $Q = s \cdot P + h \cdot P_{\text{pub}}$ and $e(P, Q)$ is the pairing between $P$ and $Q$.
...
2. Alice's public key is calculated as: 
   $P_{\text{pub}}^A = x_A \cdot P$ 
   where $P$ is the generator point of the elliptic curve.
3. Bob's private key $x_B$ is generated in the same way: $x_B \in \mathbb{Z}_r$, and his public key is calculated as: 
   $P_{\text{pub}}^B = x_B \cdot P$
...
1. Alice calculates the shared point 
   $P_{\text{shared}}^A = x_A \cdot P_{\text{pub}}^B$ 
   using her private key and Bob's public key.
2. Bob calculates the shared point 
   $P_{\text{shared}}^B = x_B \cdot P_{\text{pub}}^A$ 
   using his private key and Alice's public key.

In an ideal system, both shared points $P_{\text{shared}}^A$ and $P_{\text{shared}}^B$ will be identical, as the elliptic curve point multiplication is commutative: 
$P_{\text{shared}}^A = P_{\text{shared}}^B$
...
1. Alice and Bob calculate the bilinear pairing 
   $e(P_{\text{shared}}, P_{\text{pub}})$ 
   where $P_{\text{shared}}$ is the shared point calculated by both.
...
1. The secret key is derived as: 
   $H(P_{\text{shared}})$
