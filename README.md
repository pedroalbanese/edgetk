...
TLCP (Transport Layer Security Protocol, defined by the Chinese standard GB/T 38636-2020) is the Chinese equivalent of TLS, with mandatory support for national cryptographic algorithms: SM2 (elliptic curves for signature and key exchange), SM3 (hash), and SM4 (block cipher). Unlike traditional TLS, TLCP requires the server to possess two distinct key pairs and certificates: one for signature (SM2) and another for encryption (SM2). The encryption key pair is generated by the Certification Authority (CA), not by the server — the CA generates the encryption private key and delivers it to the requester along with both certificates, making TLCP incompatible with standard TLS.

<details>
  <summary>Transport Layer Security Protocol</summary>  

**Summary instructions for TLCP connection:**

1. Requester generates a signing key pair and submits a CSR to the CA.
2. CA issues the signature certificate based on the received CSR.
3. CA generates a new encryption key pair and creates a corresponding CSR using the original CSR as a template.
4. CA issues the encryption certificate and returns to the requester: the signature certificate, the encryption certificate, and the encryption private key.
