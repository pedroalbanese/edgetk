...
$\text{BignV1: Compute }  R = [k]G; s_0$ must be a root of $h(OID(H) \parallel R \parallel H(X)),$ 
...
1. The private key is a random number \( x \in \mathbb{Z}_r \), where \( r \) is the order of the cyclic group \( G_1 \). 
2. The public key is generated as: \( P_{\text{pub}} = x \cdot P \), where \( P \) is a generator point of the elliptic curve in group \( G_1 \).

#### 2. Signing

To sign a message \( m \), the signing process involves the following steps:

1. **Message Hash:** A hash function \( H(m) \) is used to generate a value \( h \in \mathbb{Z}_r \).
2. **Random Number Selection:** A random number \( k \in \mathbb{Z}_r \) is chosen, and the point \( R = k \cdot P \in G_1 \) is calculated.
3. **Pairing:** The pairing \( e_1 = e(R, P_{\text{pub}}) \) is computed, where \( e \) is the bilinear pairing between \( G_1 \) and \( G_2 \).
4. **Signature Generation:** The signature \( (R, s) \) is generated with the formula \( s = k - x \cdot h \pmod{r} \).

#### 3. Verification

To verify the signature \( (R, s) \) of a message \( m \), the process involves the following steps:

1. Calculate the hash value \( h = H(m) \).
2. Verify the following pairing equation: \( e(R, P_{\text{pub}}) = e(P, Q) \cdot e(P, R)^{-h} \), where \( Q = s \cdot P + h \cdot P_{\text{pub}} \) and \( e(P, Q) \) is the pairing between \( P \) and \( Q \).

If the equation holds, the signature is valid.

### Secret Key Derivation with SM9

The secret key derivation in SM9 follows a process similar to **ECDH** (Elliptic Curve Diffie-Hellman), but using the specific operations of the SM9 algorithm with elliptic curves and bilinear pairings. This process is used for two parties (Alice and Bob, for example) to generate a shared secret key, even without exchanging sensitive information directly.

#### 1. Generation of Private and Public Keys

Each party generates their private key randomly and then calculates the corresponding public key using a generator point of the elliptic curve \( P \) from group \( G_1 \) (from SM9):

1. Alice's private key \( x_A \) is randomly generated such that \( x_A \in \mathbb{Z}_r \), where \( r \) is the order of the cyclic group \( G_1 \).
2. Alice's public key is calculated as: \( P_{\text{pub}}^A = x_A \cdot P \), where \( P \) is the generator point of the elliptic curve.
3. Bob's private key \( x_B \) is generated in the same way: \( x_B \in \mathbb{Z}_r \), and his public key is calculated as: \( P_{\text{pub}}^B = x_B \cdot P \).

#### 2. Exchange of Public Keys

After generating their public keys, Alice and Bob exchange their public keys \( P_{\text{pub}}^A \) and \( P_{\text{pub}}^B \) with each other. These public keys will be used to derive the shared secret key.

#### 3. Calculation of the Shared Point

Each party uses their private key and the other party's public key to calculate the shared point. The shared point is derived as follows:

1. Alice calculates the shared point \( P_{\text{shared}}^A = x_A \cdot P_{\text{pub}}^B \), using her private key and Bob's public key.
2. Bob calculates the shared point \( P_{\text{shared}}^B = x_B \cdot P_{\text{pub}}^A \), using his private key and Alice's public key.

In an ideal system, both shared points \( P_{\text{shared}}^A \) and \( P_{\text{shared}}^B \) will be identical, as the elliptic curve point multiplication is commutative: \( P_{\text{shared}}^A = P_{\text{shared}}^B \).

#### 4. Derivation of the Secret Key

With the shared point calculated, the secret key can be safely derived. In SM9, the bilinear pairing between groups \( G_1 \) and \( G_2 \) is used to ensure the security and uniqueness of the secret key:

1. Alice and Bob calculate the bilinear pairing \( e(P_{\text{shared}}, P_{\text{pub}}) \), where \( P_{\text{shared}} \) is the shared point calculated by both.
2. In a real SM9 implementation, the bilinear pairing \( e \) between \( G_1 \) and \( G_2 \) would be used to derive a value that serves as the shared secret key.

#### 5. Derivation of the Secret Key with a Hash Function

After calculating the shared point, Alice and Bob can derive the secret key from the point \( P_{\text{shared}} \), using a hash function to ensure the uniformity and security of the derived key:

1. The secret key is derived as: \( H(P_{\text{shared}}) \)
</details>
